...
Description: "Cross version extension for CarePlan.activity from FHIR version 4.0.1 to FHIR version 3.0.1"
Context:     CarePlan
...
* extension[detail-]
  ...
  * extension[code-]
    * value[x] only CodeableConcept
    * valueCodeableConcept from CrossVersion-ProcedureCodes(SNOMEDCT) (example)
...
* extension[reference-]
  * value[x] only Reference
  * valueReference only Reference( Appointment or CommunicationRequest or DeviceRequest or MedicationRequest or NutritionOrder or Task or VisionPrescription or RequestGroup)
