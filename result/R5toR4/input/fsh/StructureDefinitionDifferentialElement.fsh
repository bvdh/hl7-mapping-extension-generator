Extension:   StructureDefinitionDifferentialElement
Id:          extension-StructureDefinition.differential.element
Title:       "StructureDefinition Differential Element "
Description: "Cross version extension for StructureDefinition.differential.element from FHIR version 5.0.0 to FHIR version 4.0.1"
Context:     StructureDefinition.differential
* extension contains elementdefinition- 0..*
* extension[elementdefinition-]
  * extension contains binding- 0..* and isModifier- 0..* and meaningWhenMissing- 0..* and sliceName- 0..* and mustSupport- 0..* and code- 0..* and mapping- 0..* and representation- 0..* and maxValue- 0..* and constraint- 0..* and isSummary- 0..* and pattern- 0..* and sliceIsConstraining- 0..* and isModifierReason- 0..* and short- 0..* and contentReference- 0..* and slicing- 0..* and definition- 0..* and defaultValue- 0..* and min- 0..* and type- 0..* and label- 0..* and base- 0..* and valueAlternatives- 0..* and path- 0..* and maxLength- 0..* and mustHaveValue- 0..* and id- 0..* and alias- 0..* and condition- 0..* and orderMeaning- 0..* and fixed- 0..* and minValue- 0..* and comment- 0..* and max- 0..* and example- 0..* and requirements- 0..*
  * extension[binding-]
  ...
  * extension[maxValue-]
    * value[x] only date or dateTime or string or unsignedInt or Quantity or time or integer or positiveInt or decimal or instant
  ...
  * extension[base-]
    ...
    * extension[max-]
      * value[x] only string
  * extension[valueAlternatives-]
    * value[x] only canonical
  * extension[path-]
    * value[x] only string
  * extension[maxLength-]
    * value[x] only integer
  * extension[mustHaveValue-]
    * value[x] only boolean
  * extension[id-]
    * value[x] only id
  * extension[alias-]
